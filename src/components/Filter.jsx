const Filter = ({setFilter, filter, fill}) => { 
    return (
        ...
        <div className={` p-4 rounded-md flex justify-center gap-4 transition-all duration-300 ${fill !== "#000" ? "bg-gray-800 text-white" : "text-gray-600 bg-white"}`}>
        <button onClick={()=> setFilter("all")} className={`${filter === "all" ? "text-blue-500" : "text-gray-500 hover:text-blue-500"}`}>Order All </button>
